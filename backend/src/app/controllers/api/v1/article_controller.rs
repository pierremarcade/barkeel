...
pub async fn index(State(config): State<Arc<Mutex<Config>>>) -> impl IntoResponse {
    ...
    let results = menu_items::table
        ...
        .load::<ArticleWithMenu>(&mut config_guard.database.pool.get().unwrap())
        .expect("Error loading datas");
    let serialized = serde_json::to_string(&results).unwrap();
    Response{status_code: StatusCode::OK, content_type: "application/json", datas: serialized}
}

pub async fn show(Path(other_slug): Path<String>, State(config): State<Arc<Mutex<Config>>>) -> impl IntoResponse {
    ...
    match menu_items::table
        ...
        .filter(slug.eq(other_slug)).first::<(i32, String, String, String, bool, String, Option<String>)>(&mut config_guard.database.pool.get().unwrap()) {
        Ok(result) => {
...
pub async fn search(Path(query): Path<String>, State(config): State<Arc<Mutex<Config>>>) -> impl IntoResponse {
    let config_guard = config.lock().unwrap();
    let results = menu_items::table
        .inner_join(articles::table)
        .inner_join(menus::table)
