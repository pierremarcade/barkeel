...
pub async fn index(State(config): State<Arc<Mutex<Config>>>) -> impl IntoResponse  {
    let config_guard = config.lock().unwrap();
    let all_menus = menus::table.select(Menu::as_select()).order(menus::id.asc()).load(&mut config_guard.database.pool.get().unwrap());
    match all_menus {
        Ok(all_menus) => {
            let menu_items_with_articles = menu_items::table
                ...
                .load::<MenuItemWithArticle>(&mut config_guard.database.pool.get().unwrap())
...
pub async fn show(Path(param_id): Path<i32>, State(config): State<Arc<Mutex<Config>>>) -> Json<String> {
    let config_guard = config.lock().unwrap();
    let result = menus
        .find(param_id).first::<Menu>(&mut config_guard.database.pool.get().unwrap()) 
        .expect("Error loading data");
    let serialized = serde_json::to_string(&result).unwrap();
    Json(serialized)
}

pub async fn create(Json(payload): Json<Menu>, State(config): State<Arc<Mutex<Config>>>) -> Json<String> {
    let config_guard = config.lock().unwrap();
...
pub async fn update(Path(param_id): Path<i32>, Json(payload): Json<Menu>, State(config): State<Arc<Mutex<Config>>>) -> Json<String> {
    let config_guard = config.lock().unwrap();
...
pub async fn delete(Path(param_id): Path<i32>, State(config): State<Arc<Mutex<Config>>>) -> impl IntoResponse {
    let config_guard = config.lock().unwrap();
    diesel::delete(menus)
        .filter(id.eq(param_id))
